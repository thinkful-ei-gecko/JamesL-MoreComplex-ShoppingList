...
function handleEditItem(){
  $('.js-shopping-item').on('click', function(event){
    ...
    $(this).html(`<input class="editItem" value="${itemName}" >`);
  });
};
...
function renderShoppingList() {
  ...
  if(STORE.search != ''){
    filteredItems = filteredItems.filter(item => item.name.startsWith(STORE.search)
    );
  };
...
function searchItemEntry() {
  $('.js-search-entry').on('keyup', function(event){
    event.preventDefault();
    const newSearchItem = $('.js-search-entry').val();
    handleSearchEntry(newSearchItem);
  });
};

function handleSearchEntry(searchVal){
